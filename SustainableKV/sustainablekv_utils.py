...
class SustainableKV:
    ...
    def recycle_cache(self, 
        ...
        if self.merge.lower() != 'none' and able_to_merge:
            ...
            key_state_del = key_state.gather(dim=2, index=second_drop_absolute_indices)  # [B, H, num_second_dropped_tok, head_dim]
            value_state_del = value_state.gather(dim=2, index=second_drop_absolute_indices)  # [B, H, num_second_dropped_tok, head_dim]

            # 2. Get the indices of Source token (to merge from)
            max_indices = max_cos_simi_indices.gather(dim=-1, index=second_drop_indices)  # [B, H, num_second_dropped_tok] -> relative indices
            max_indices = max_indices.unsqueeze(-1).expand(-1, -1, -1, head_dim)  # [B, H, num_second_dropped_tok, head_dim]
