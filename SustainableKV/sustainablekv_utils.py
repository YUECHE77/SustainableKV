...
class SustainableKV:
    ...
    def recycle_cache(self, 
                      ...
                      recycle_budget: int):
        """
        This function contains the process of recycling and merging. We will use the key_state  
        to compute cosine similarity, and the function will eventually return the (subsequence)  
        Key and Value after recycling and merging.

        :param selected_indices: Relative position (indices) with shape [B, H, select_budget]
        :param key_state: Sliced key_state with shape [B, H, subseq_len, head_dim]
        :param value_state: Sliced value_state with shape [B, H, subseq_len, head_dim]
        :param recycle_budget: Number of tokens to be recycled

        Returns:
        Final selected and merged Key states and Value states.
        """
        bsz, num_heads, subseq_len, head_dim = key_state.shape
        ...
        if selected_indices.shape[-1] == subseq_len:
            assert recycle_budget == 0, (
                f'All the tokens in the subsequence (total {subseq_len}) are selected, '
        ...
        drop_indices = self.get_non_selected_indices(subseq_len, selected_indices, device=key_state.device, bsz=bsz, num_heads=num_heads)
        ...
        key_dropped = key_state.gather(dim=2, index=drop_indices_expand)
        key_selected = key_state.gather(dim=2, index=selected_indices_expand)  # [B, H, select_budget, head_dim]
        value_selected = value_state.gather(dim=2, index=selected_indices_expand)  # [B, H, select_budget, head_dim]

        eps = 1e-8
        key_selected_norm = torch.norm(key_selected, dim=-1, keepdim=True).clamp(min=eps)
        key_dropped_norm = torch.norm(key_dropped, dim=-1, keepdim=True).clamp(min=eps)
        cos_simi = (key_dropped / key_dropped_norm) @ (key_selected / key_selected_norm).transpose(-1, -2)  # [B, H, num_dropped_tok, select_budget]
        max_cos_simi_values, max_cos_simi_indices = cos_simi.max(dim=-1)  # both [B, H, num_dropped_tok]

        most_similar_tok_idx = max_cos_simi_indices.topk(recycle_budget, dim=-1).indices  # [B, H, recycle_budget] -> Relative Indices
        recycle_indices = drop_indices.gather(dim=-1, index=most_similar_tok_idx)  # [B, H, recycle_budget] -> Absolute Indices
        recycle_indices = recycle_indices.unsqueeze(-1).expand(-1, -1, -1, head_dim)  # [B, H, recycle_budget, head_dim]

        key_recycle = key_state.gather(dim=2, index=recycle_indices)  # [B, H, recycle_budget, head_dim]
        value_recycle = value_state.gather(dim=2, index=recycle_indices)  # [B, H, recycle_budget, head_dim]

        # ------------------------------ Merging ------------------------------
        able_to_merge = (recycle_budget + selected_indices.shape[-1] < subseq_len)

        if self.merge.lower() != 'none' and able_to_merge:
            assert drop_indices.shape[-1] == max_cos_simi_indices.shape[-1], (
                f'Inconsistant number of dropped tokens.'
                f'Got {drop_indices.shape[-1]} and {max_cos_simi_indices.shape[-1]}'
            )
            ...
            second_drop_indices = self.get_non_selected_indices(num_dropped_tok, most_similar_tok_idx, device=key_state.device, bsz=bsz, num_heads=num_heads)
            ...
            max_values_second_drop, max_indices_second_drop = cos_simi_second_drop.max(dim=-1)  # both [B, H, num_second_dropped_tok]
            threshold = max_values_second_drop.mean()
            filter_indices = (max_values_second_drop.mean(1) >= threshold).squeeze(0)  # bool: [num_second_dropped_tok, ]
            # -------------------------------

            second_drop_absolute_indices = drop_indices.gather(dim=-1, index=second_drop_indices)  # [B, H, num_second_dropped_tok]
            second_drop_absolute_indices = second_drop_absolute_indices[..., filter_indices]
            second_drop_absolute_indices = second_drop_absolute_indices.unsqueeze(-1).expand(-1, -1, -1, head_dim)  # [B, H, num_second_dropped_tok, head_dim]

            key_state_del = key_state.gather(dim=2, index=second_drop_absolute_indices)  # [B, H, filtered_second_dropped_tok, head_dim]
            value_state_del = value_state.gather(dim=2, index=second_drop_absolute_indices)  # [B, H, filtered_second_dropped_tok, head_dim]

            # 2. Get the indices of Source token (to merge from)
            max_indices = max_cos_simi_indices.gather(dim=-1, index=second_drop_indices)  # [B, H, filtered_second_dropped_tok] -> relative indices
            max_indices = max_indices[..., filter_indices]
            max_indices = max_indices.unsqueeze(-1).expand(-1, -1, -1, head_dim)  # [B, H, filtered_second_dropped_tok, head_dim]

            if self.merge == 'pivot':
                key_state_src = key_selected.gather(dim=2, index=max_indices)  # The source (Key) to merge: [B, H, num_second_dropped_tok, head_dim]
                value_state_src = value_selected.gather(dim=2, index=max_indices)  # The source (Value) to merge  # [B, H, num_second_dropped_tok, head_dim]

                # Apply pivot merging
                key_merge = (key_state_src + key_state_del) / 2  # [B, H, num_second_dropped_tok, head_dim]
                value_merge = (value_state_src + value_state_del) / 2  # [B, H, num_second_dropped_tok, head_dim]

            elif self.merge == 'average':
                # Apply average merging
                key_merge = key_state_del  # [B, H, num_second_dropped_tok, head_dim]
                value_merge = value_state_del  # [B, H, num_second_dropped_tok, head_dim]
                
            elif self.merge == 'weighted':
                max_values = max_cos_simi_values.gather(dim=-1, index=second_drop_indices).unsqueeze(-1)  # [B, H, num_second_dropped_tok, 1]

                # Apply weighted merging
                key_merge = key_state_del * max_values
                value_merge = value_state_del * max_values
            else:
                raise ValueError(f'Currently only support pivot merging, but you are trying to use: {self.merge}')
        ...
        compressed_key_state = torch.concat([key_selected, key_recycle], dim=-2)
        compressed_value_state = torch.concat([value_selected, value_recycle], dim=-2)

        return compressed_key_state, compressed_value_state
    ...
    def update_kv(self,
        ...
        if q_len < self.desired_cache_size:
            ...
            for budget in all_subseq_budget:
                # Potential edge case: last_seq_len=1 -> recycle_budget=0, select_budget=1
                recycle_tok_budget = int(self.recycling_percent * budget)
                all_subseq_recyele_budget.append(recycle_tok_budget)  # number of recycled tokens
                all_subseq_select_budget.append(budget - recycle_tok_budget)  # number of selected tokens

            # 3. Get important tokens' KV (select, recycle, and merge) within each subsequence
            all_subseq_key_states = [key_states[:, :, :self.attn_sink_tok, :], ]
            all_subseq_value_states = [value_states[:, :, :self.attn_sink_tok, :], ]
            ...
            for i in range(num_subseq):
                ...
                # Recycling and Merging happen in here
                compressed_key_state, compressed_value_state = self.recycle_cache(
                    selected_indices=selected_indices,
                    key_state=key_states[:, :, candidates_start : candidates_start + subseq_len, :],
                    value_state=value_states[:, :, candidates_start : candidates_start + subseq_len, :],
                    recycle_budget=subseq_recycle_budget,
                )

                all_subseq_key_states.append(compressed_key_state)
                all_subseq_value_states.append(compressed_value_state)
            
            # 4. Always include the obs window
            all_subseq_key_states.append(key_states[:, :, -self.window_size:, :])
            all_subseq_value_states.append(value_states[:, :, -self.window_size:, :])

            # ----------------------------------------------------------------------------------

            key_states = torch.cat(all_subseq_key_states, dim = 2)  # [B, H, desired_cache_size, head_dim]
            value_states = torch.cat(all_subseq_value_states, dim = 2)  # [B, H, desired_cache_size, head_dim]
...
def init_sustainablekv(self):
    ...
    if not hasattr(self, 'cache_killer'):
        ...
        if not hasattr(self.config, 'recycling_percent'):
            ...
            self.config.recycling_percent = 0.1
        if not hasattr(self.config, 'merge'):
            self.config.merge = 'average'  # pivot, average, weighted, or None
