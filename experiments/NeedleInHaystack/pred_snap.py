...
class LLMNeedleHaystackTester:
    ...
    def reset_kv_args(self):
        if self.use_compress:
            ...
            all_max_capacity_prompt = [1024] * num_layers
            pooling = 'maxpool'
            all_kernel_size = [5] * num_layers

            for i in range(num_layers):
                self.model_to_test.model.layers[i].self_attn.config.window_size = all_window_size[i]
                self.model_to_test.model.layers[i].self_attn.config.max_capacity_prompt = all_max_capacity_prompt[i]
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        #     if not os.path.exists(f'snapkv_results_needle/contexts'):
        #         os.makedirs('snapkv_results_needle/contexts')

        #     if not os.path.exists(f'snapkv_results_needle/contexts/{self.model_name}'):
        #         os.makedirs(f'snapkv_results_needle/contexts/{self.model_name}')

        #     with open(f'snapkv_results_needle/contexts/{self.model_name}/{context_file_location}_context.txt', 'w') as f:
        #         f.write(context)
        
        if self.save_results:
            # Save the context to file for retesting
            # if not os.path.exists(f'snapkv_results_needle/results'):
            #     os.makedirs(f'snapkv_results_needle/results')
            
            # if not os.path.exists(f'snapkv_results_needle/results/{self.model_name}'):
            #     os.makedirs(f'snapkv_results_needle/results/{self.model_name}')
    ...
    def result_exists(self, context_length, depth_percent):
        ...
        """
        # results_dir = 'snapkv_results_needle/results/' + self.model_name
...
if __name__ == "__main__":
    ...
    ht = LLMNeedleHaystackTester(
        needle=needle,
        haystack_dir=r'/root/KVCache_Factory/KVCache-Factory-main/data/PaulGrahamEssays',
