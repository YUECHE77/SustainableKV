...
def merge_kv(key_states, value_states, indices, window_size, merge):
    ...
    all_indices_flattened = all_indices.flatten()  # [bsz * num_heads * k_len]

    selected_indices_flattened = indices.flatten()  # [bsz * num_heads * topk_len] -> 错了吧 bsz * num_heads * topk_len * head_dim
...
class SnapKVCluster():
    def __init__(self, window_size = 64, max_capacity_prompt = 256 + 64, kernel_size = 5, pooling = 'avgpool', merge = False):
    ...
    def reset(self, window_size = 64, max_capacity_prompt = 256 + 64, kernel_size = 5, pooling = 'avgpool', merge = False):
        self.window_size = window_size
        self.max_capacity_prompt = max_capacity_prompt
        assert self.max_capacity_prompt - self.window_size > 0
    ...
    def update_kv(self, key_states, query_states, value_states, attention_mask, num_key_value_groups):
        ...
        if q_len < self.max_capacity_prompt:
            ...
            attn_weights = torch.matmul(query_states[..., -self.window_size:, :], key_states.transpose(2, 3)) / math.sqrt(head_dim)  # [B, H, window_size, seq_len]
            ...
            attn_weights[:, :, -self.window_size:, -self.window_size:] += attention_mask  # 把 obs_window 与它自己的 attn 用很大的数字mask掉了
            attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
            attn_weights_sum = attn_weights[:, :, -self.window_size:, :-self.window_size].sum(dim=-2)  # [B, H, window_size, seq_len - window_size].sum(-2) = [B, H, seq_len - window_size]

            if self.pooling == 'avgpool':
                attn_cache = F.avg_pool1d(attn_weights_sum, kernel_size=self.kernel_size, padding=self.kernel_size//2, stride=1)  # [B, H, seq_len - window_size]
            elif self.pooling == 'maxpool':
                attn_cache = F.max_pool1d(attn_weights_sum, kernel_size=self.kernel_size, padding=self.kernel_size//2, stride=1)
            else:
                raise ValueError('Pooling method not supported')
            
            indices = attn_cache.topk(self.max_capacity_prompt - self.window_size, dim=-1).indices  # [B, H, max_cache_size - window_size]
            indices = indices.unsqueeze(-1).expand(-1, -1, -1, head_dim)  # [B, H, max_cache_size - window_size, head_dim]
