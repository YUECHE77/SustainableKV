...
def mistral_flash_attn2_forward(
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if hasattr(self, "kv_seq_len"): #[SnapKV] add kv_seq_len in prepare_inputs_for_generation_mistral()
            if self.kv_seq_len != 0:  # first generation = 0, second = 17395 (previous seq_len)
    ...
    if past_key_value is not None:
        ...
        cache_has_contents = past_key_value.get_seq_length(self.layer_idx) > 0  # second generation: past_key_value.key_cache[0].shape = [B, H, max_cache_size, head_dim]
